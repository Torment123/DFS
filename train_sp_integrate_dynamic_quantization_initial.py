...
model_names = sorted(name for name in models_initial.__dict__
                     if name.islower() and not name.startswith('__')
                     and callable(models_initial.__dict__[name])
                     )
...
def parse_args():
    ...
    parser.add_argument('--eval-every', default=390, type=int,
...
def run_training(args):
    # create model
    model = models_initial.__dict__[args.arch](args.pretrained)
    ...
    # costs = [1/64, 1/16, 1/4, 1]
    # costs = [0,1/16,1/4,1]

    # bits = [8,16,0]
    costs = [0, 1/16, 25/256, 9/64, 49/256, 1/4, 1]
    bits = [8,10,12,14,16,0]
    # costs = [0, 1/256, 1/64, 9/256, 1/16, 25/256, 9/64, 49/256, 1/4, 1]
   
    # bits = [2,4,6,8,10,12,14,16,0]
    ...
    for i in range(args.start_iter, args.iters):
        ...
        # compute output
        output, masks = model(input_var, bits)
        
        computation_cost = 0
        computation_all = 0
        
        for layer in range(network_depth - 1):
            ...
            for k in range(len(costs)):
                ...
                layerwise_decision_statistics[layer][k].update(ratio, 1)
                
                computation_cost += masks[layer][k].sum() * costs[k]
            
        
        # for layer in range(network_depth - 1):
            # computation_cost += masks[layer].sum()
            
            # computation_all += reduce((lambda x, y: x * y), masks[layer].shape)
            
        cp_ratio = (float(computation_cost) / float(computation_all)) * 100
        ...
        if i % args.print_freq == 0 or i == (args.iters - 1):
            logging.info("Iter: [{0}/{1}]\t"
                         ...
                         "Computation_Percentage: {cp_record.val:.3f}({cp_record.avg:.3f})\t".format(
                            i,
                            args.iters,
