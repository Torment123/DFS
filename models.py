...
class BasicBlock(nn.Module):
    ...
    def forward(self, x, bit):
        ...
        out = self.conv1(x, bit)
        out = self.bn1(out)
        out = self.relu(out)

        out = self.conv2(out, bit)
        out = self.bn2(out)

        if self.downsample is not None:
            residual = self.downsample(x, bit)
...
class _ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = QConv2d(self.inplanes, planes * block.expansion, kernel_size=1, stride=stride, bias=False, num_bits=NUM_BITS, num_bits_weight=NUM_BITS_WEIGHT, num_bits_grad=NUM_BITS_GRAD, biprecision=BIPRECISION)
    ...
    def forward(self, x, bit):
        ...
        for g in range(3):
            for i in range(self.num_layers[g]):
                x = getattr(self, 'group{}_layer{}'.format(g+1, i))(x, bit)
...
class RNNGate(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, rnn_type='lstm'):
        ...
        # reduce dim
        self.proj = nn.Linear(hidden_dim, 7)
...
class ResNetRecurrentGateSP(nn.Module):
    ...
    def forward(self, x, bits):
        ...
        # self.control.hidden_two = self.control.init_hidden(batch_size)

        masks = []
        # gprobs = []
        # must pass through the first layer in first group
        x = getattr(self, 'group1_layer0')(x, 0)
        # gate takes the output of the current layer

        gate_feature = getattr(self, 'group1_gate0')(x)
        mask_one = self.control(gate_feature)
        ...
        for g in range(3):
            for i in range(0 + int(g == 0), self.num_layers[g]):
                if getattr(self, 'group{}_ds{}'.format(g+1, i)) is not None:
                    prev = getattr(self, 'group{}_ds{}'.format(g+1, i))(prev, 0)
                ...
                for k in range(len(bits)):
                    out = getattr(self, 'group{}_layer{}'.format(g+1, i))(x, bits[k])
                    output_candidates.append(out)
                    
                # x = getattr(self, 'group{}_layer{}'.format(g+1, i))(x, 0)
                    
                mask_list = []
                
                # mask_list.append(1 - mask_one)
                
                # for j in range(4):
                    # mask_list.append(mask_one * mask_two[:,j,:,:,:])
                    
                for j in range(len(bits) + 1):
                    mask_list.append(mask_one[:,j,:,:,:])
                    
                # prev = x = sum([mask_list[k].expand_as(out) * output_candidates[k] for k in range(5)])
                
                prev = x = sum([mask_list[k].expand_as(out) * output_candidates[k] for k in range(len(bits) + 1)])
...
def cifar10_rnn_gate_20(pretrained=False, **kwargs):
    model = ResNetRecurrentGateSP(BasickBlock, [3, 3, 3], num_classes=10,
                                  embed_dim=10, hidden_dim = 10)
    return model

def cifar10_rnn_gate_31(pretrained=False, **kwargs):
    model = ResNetRecurrentGateSP(BasicBlock, [5, 5, 5], num_classes=10,
                                  embed_dim=10, hidden_dim=10)
    return model
